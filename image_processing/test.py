...
def test_inverted_2():
    src = lab.load_greyscale_image('cat_grey.png')
    result = lab.inverted(src)
    expected = lab.load_greyscale_image('cat_grey_inverted.png')
    compare_images(result, expected)
...
def test_blurred_black_image():
    ...
    src = {
        'height': 6,
        'width': 5,
        'pixels': [0 for _ in range(5 * 6)]
    }

    res = lab.blurred(src, 3)
    compare_images(src, res)



def test_blurred_centered_pixel():
    # REPLACE THIS with your 2nd test case from section 5.1
    src = lab.load_greyscale_image('test_images/centered_pixel.png')
    res = lab.blurred(src, 3)
    expected = {
        'height': 11,
        'width': 11,
...
def test_edges_centered_pixel():
    ...
    src = lab.load_greyscale_image('test_images/centered_pixel.png')
    res = lab.edges(src)
    expected = lab.load_greyscale_image('edge_centered_pixel.png')
